...
- [ ] Explain what a token is used for.
  ...
  A token is used to identify a particular client that has passed authentication with a sever. The server reads the token and make decisions for data transfer based on the client’s permission. 

- [ ] What steps can you take in your web apps to keep your data secure?

We can protect our apps with tokens passed in the header of our axios calls as well as protected routes in our react-router. The protected routes should only render when we have the proper authentication and authorization from the server. Having server data locked behind authentication and tokens generated by the server instead of hard coded into the api would also increase security.

- [ ] Describe how web servers work.

A web server hosts web content. It also receives a request for a web page sent from a browser and shows the proper page that was requested. The web server takes care of the hosting and maintaining the code that allows the server to be connected to the internet. 

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

Create= post. Post creates a new resource in the data that we are accessing. Read = get. When we pull data with a request that is the read operation of CRUD. This lets us access and display data in the application. Update: put. With a put request we are altering/ updating data that is already on the server. Delete = delete. This http method will delete an entry in the data or app that we are accessing. Using all of these we can create a dynamic application that can be modified by a user.
